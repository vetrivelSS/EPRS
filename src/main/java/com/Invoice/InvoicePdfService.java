...
public class InvoicePdfService {
    public byte[] generateInvoicePdf(Map<String, Object> inv, List<Map<String, Object>> items) {
        ...
        metaTable.getDefaultCell().setMinimumHeight(20); 
        ...
        Object rawDate = inv.getOrDefault("created_at", null);
        String formattedDate = "-";

        if (rawDate instanceof LocalDateTime) {
            formattedDate = ((LocalDateTime) rawDate).format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
        } else if (rawDate != null) {
            // Fallback if it's already a String or another type
            formattedDate = String.valueOf(rawDate).substring(0, 10);
        }

        metaTable.addCell(new Phrase("Delivery Note Date:\n" + formattedDate, normalFont));
        ...
        double calcTaxableTotal = 0.0;
        double calcQtyTotal = 0.0;
        java.util.Set<String> uniqueHsns = new java.util.LinkedHashSet<>();
        for (int i = 0; i < items.size(); i++) {
            ...
            Object w = inv.get("width"); if (w == null) w = inv.get("WIDTH");
            Object l = inv.get("length"); if (l == null) l = inv.get("LENGTH");
            Object t = inv.get("thickness"); if (t == null) t = inv.get("THICKNESS");
            Object q = inv.get("thickness"); if (t == null) t = inv.get("QUANTITYNUMBER");


            String width = (w != null) ? String.valueOf(w) : "0";
            String length = (l != null) ? String.valueOf(l) : "0";
            String thickness = (t != null) ? String.valueOf(t) : "0";            
            String quantity_no = (q != null) ? String.valueOf(q) : "0";

            String fullDescription = String.format("%s _ %s x %s x %smm - %s Sheets",
                    material, length, width, thickness, quantity_no);

            // 3. Add to Table
            itemTable.addCell(createItemCell(fullDescription, normalFont, Element.ALIGN_LEFT));

            Object hsnObj = item.get("hsn_code"); // Check item JSON first

            if (hsnObj == null || hsnObj.toString().equals("0") || hsnObj.toString().isEmpty()) {
                hsnObj = inv.get("hsn_code");
                if (hsnObj == null) {
                    hsnObj = inv.get("HSN_CODE"); 
                }
            }

            String hsnCode = (hsnObj != null) ? hsnObj.toString() : "-";
            if (!hsnCode.equals("-")) {
                uniqueHsns.add(hsnCode); // Collect for the bottom table
            }
            itemTable.addCell(createItemCell(hsnCode, normalFont, Element.ALIGN_CENTER));

     // --- MOVE CALCULATIONS HERE (Only once) ---
            double qty = Double.parseDouble(item.getOrDefault("quantityKg", "0").toString());
            double amt = Double.parseDouble(item.getOrDefault("totalAmount", "0").toString());
            calcQtyTotal += qty;
            calcTaxableTotal += amt;

            itemTable.addCell(createItemCell(qty + " Kg", normalFont, Element.ALIGN_RIGHT));
            itemTable.addCell(createItemCell(String.valueOf(item.getOrDefault("ratePer", "0")), normalFont, Element.ALIGN_RIGHT));
            itemTable.addCell(createItemCell("Kg", normalFont, Element.ALIGN_CENTER));
            itemTable.addCell(createItemCell(String.format("%.2f", amt), normalFont, Element.ALIGN_RIGHT));
        }

        // Calculations
        double cgstAmt = calcTaxableTotal * 0.09;
        double sgstAmt = calcTaxableTotal * 0.09;
        double subTotal = calcTaxableTotal + cgstAmt + sgstAmt;
        double roundedTotal = Math.round(subTotal);
        double roundOff = roundedTotal - subTotal;

        // Add Tax Rows to Item Table
        addCleanTaxRow(itemTable, "Total", String.format("%.2f", calcTaxableTotal), boldFont);
        addCleanTaxRow(itemTable, "CGST 9%", String.format("%.2f", cgstAmt), normalFont);
        addCleanTaxRow(itemTable, "SGST 9%", String.format("%.2f", sgstAmt), normalFont);
        addCleanTaxRow(itemTable, "Round Off", String.format("%.2f", roundOff), normalFont);
        ...
        itemTable.addCell(createItemCell(String.format("%.2f Kg", calcQtyTotal), boldFont, Element.ALIGN_RIGHT));
        itemTable.addCell(new PdfPCell(new Phrase("")));
        itemTable.addCell(new PdfPCell(new Phrase("")));
        itemTable.addCell(createItemCell("₹ " + String.format("%.2f", roundedTotal), boldFont, Element.ALIGN_RIGHT));
        ...
        wordsCell.addElement(new Phrase("Indian Rupees " + convertToWords(String.valueOf(roundedTotal)) + " Only", boldFont));
        ...
        taxTable.addCell(createHeaderCell("HSN/SAC", smallBoldFont));
        taxTable.addCell(createHeaderCell("Taxable Value", smallBoldFont));
        taxTable.addCell(createNestedTaxHeader("Central Tax", smallBoldFont));
        taxTable.addCell(createNestedTaxHeader("State Tax", smallBoldFont));
        taxTable.addCell(createHeaderCell("Total Tax", smallBoldFont));

        // HSN Row
//        String hsnList = String.join(", ", uniqueHsns);
        String allHsns = uniqueHsns.isEmpty() ? "-" : String.join(", ", uniqueHsns);
        taxTable.addCell(createItemCell(allHsns, normalFont, Element.ALIGN_CENTER));
        taxTable.addCell(createItemCell(String.format("%.2f", calcTaxableTotal), normalFont, Element.ALIGN_RIGHT));
        
        // CGST Nested Data	
        PdfPTable cgstData = new PdfPTable(2);
        cgstData.addCell(createItemCell("9%", smallFont, Element.ALIGN_CENTER));
        cgstData.addCell(createItemCell(String.format("%.2f", cgstAmt), smallFont, Element.ALIGN_RIGHT));
        PdfPCell cDataCell = new PdfPCell(cgstData); cDataCell.setPadding(0);
        taxTable.addCell(cDataCell);

        // SGST Nested Data
        PdfPTable sgstData = new PdfPTable(2);
        sgstData.addCell(createItemCell("9%", smallFont, Element.ALIGN_CENTER));
        sgstData.addCell(createItemCell(String.format("%.2f", sgstAmt), smallFont, Element.ALIGN_RIGHT));
        PdfPCell sDataCell = new PdfPCell(sgstData); sDataCell.setPadding(0);
        taxTable.addCell(sDataCell);

        taxTable.addCell(createItemCell(String.format("%.2f", (cgstAmt + sgstAmt)), normalFont, Element.ALIGN_RIGHT));
      ...
      panPara.add(new Phrase("AAMPS5157N", smallFont));
      ...
      document.add(box1Table);
        ...
        signatureBoxTable.setSpacingBefore(0); 
    ...
    private PdfPCell createNestedTaxHeader(String title, Font font) {
        PdfPTable nested = new PdfPTable(1);
        nested.setWidthPercentage(100);
        
        // Top Part: "Central Tax" or "State Tax"
        PdfPCell tCell = new PdfPCell(new Phrase(title, font));
        tCell.setBackgroundColor(Color.LIGHT_GRAY);
        tCell.setHorizontalAlignment(Element.ALIGN_CENTER);
        tCell.setPadding(2);
        nested.addCell(tCell);
        
        // Bottom Part: Rate | Amount
        PdfPTable sub = new PdfPTable(2);
        sub.setWidthPercentage(100);
        sub.addCell(createHeaderCell("Rate", font));
        sub.addCell(createHeaderCell("Amount", font));
        
        PdfPCell subCell = new PdfPCell(sub);
        subCell.setPadding(0);
        nested.addCell(subCell);

        PdfPCell main = new PdfPCell(nested);
        main.setPadding(0);
        return main;
    }
